	...
	$('#here').innerHTML=a.split('\n').filter(a=>!!a).map(a=>a.split('|')).map(a=>
			...
			(a[1]=='d')?(`<a class="dir" href="#${dir}${a[0]}/|${at}">${a[0]}</a>`):
				($('#file_show').checked)?(
					...
					`<a class="file" href="#${dir}|${at}|${a[0]}"
					      data-href="/cgi-bin/api.sh?|get|${dir}|${a[2]}|${a[0]}"
					${pass?`data-download="#${dir}|${at}|${a[0]}|${a[2]}"`
					:`data-download="/cgi-bin/api.sh?|get|${dir}|${a[2]}|${a[0]}"`}
					>${a[0]}</a>`
					):pass?(
						// downlad passworded file
						`<a class="file" href="#${dir}|${at}|${a[0]}|${a[2]}"
						        data-download="#${dir}|${at}|${a[0]}|${a[2]}"
						data-href="/cgi-bin/api.sh?|get|${dir}|${a[2]}|${a[0]}"
...
fileDetails=(name)=>{
	var freq={0:'Now',1:'Monthly',5:'Weekly',30:'Daily',720:'Hourly',8640:'5min'};
...
function startGallery(){
	...
	// var masks=prompt('What files to show',masks);
	// if(!masks) return;
	...
	var elements=[...$('#here').children].filter(x=>x.className=='file' && masks.some(rx=>rx.test(x.innerText)))
		// TODO: if pass, then download URL
		.map(el=>{return {src: el.dataset.href, thumb:el.dataset.href, download:el.innerText, downloadUrl:el.dataset.download, subHtml:`<p>${el.innerText}</p>`}});
